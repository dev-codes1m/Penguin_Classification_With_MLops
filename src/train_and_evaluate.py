...
def train_and_evaluate(config_path):
    ...
    multi_soft = config["estimators"]["XGBClassifier"]["params"]["multi_soft"]
    ...
    with mlflow.start_run(run_name= mlflow_config["run_name"]) as mlops_run:
        lr = XGBClassifier(max_depth = max_depth,random_state=random_state,booster=booster,objective=multi_soft)
        lr.fit(np.array(train_x),np.array(train_y))
        predicted_qualities = lr.predict(np.array(test_x))
        print(predicted_qualities)
        accuracy_score = eval_metrics(test_y,predicted_qualities)
